...
**Routing implication for P07:** preserve Benin as evidence that constraint diagnosis, treatment design, outcome choice, and time horizon matter. For similar target populations, compare skill/management support against capital support only when delivery quality, participant baseline technical skills, gender constraints, grant design, cost accounting, and persistence are sufficiently comparable. Do not turn these Benin arm estimates into a portable global modality ranking.

Confidence: **high** on the within-study direction and reported component costs; **moderate** on any mechanism/binding-constraint interpretation even within this study; **low** on portability to different entrepreneurship programs, populations, grant sizes, training quality, or macroeconomic settings.
...
- **START:** use Benin as a within-study comparison of specific skill/management-support and capital-support packages, while treating the stronger mechanism interpretation as a hypothesis to test rather than an identified binding constraint.
